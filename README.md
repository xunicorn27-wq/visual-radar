...
3. 让 Codex 读取 Agent batch，并将符合格式的结果写入 `data/visual_radar_agent_output.json`。
4. 导入 Codex 输出：

   ```bash
   npm run agent:import
   ```
...
9. 在本地 Express 服务调用当期 `send-wecom?dryRun=1`，检查实际企业微信 Markdown。只有用户明确确认后，才能执行不带 `dryRun=1` 的正式发送。
...
GitHub Pages 工作流不需要 GitHub Actions Secrets。`OPENAI_API_KEY`、`WECOM_BOT_WEBHOOK` 和 `CRON_SECRET` 都不得加入 Pages workflow；Webhook 和管理密钥只保存在本地 `.env.local`。
...
| `VISUAL_RADAR_PUBLIC_URL` | Pages 项目地址，不带结尾斜杠 |
